...
import tensorflow as tf             # 用的是tf1.1.0
# 若安装的是tf2.0，则可以尝试下列代码
...
# #利用feed_dict来改变x的值
# x = tf.placeholder(tf.int64,name="x") # placehodler占位符 # 初始化变量
# print(sess.run(2 * x,feed_dict={x:3}))                   # 运行session后打印
# sess.close()
...
# 对于get_variable()来说，对于已经创建的变量对象，就把那个对象返回；如果没有创建的话，就创建一个新的.
...
def model(X_train, Y_train, X_test, Y_test,
   ...
   （将minibatch_size从32改为64，运行时间更短，正确率上升从72.5%到88.3%）
